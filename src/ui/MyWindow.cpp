...
MyWindow::DebugStreambuf::DebugStreambuf(MyWindow* window)
    : window_(window) {}

MyWindow::DebugStreambuf::int_type MyWindow::DebugStreambuf::overflow(int_type c) {
      ...
      debug_buffer_(Gtk::TextBuffer::create())
{
    ...
    send_button_.signal_clicked().connect(sigc::mem_fun(*this, &MyWindow::on_send_clicked));
    ...
    mqtt_connect_button_.signal_clicked().connect(sigc::mem_fun(*this, &MyWindow::on_mqtt_connect_clicked));
    ...
    open_folder_button_.signal_clicked().connect(sigc::mem_fun(*this, &MyWindow::on_open_folder_clicked));
    ...
    // Connect signals
    sauron_eye_panel_.signal_capture_taken().connect(sigc::mem_fun(*this, &MyWindow::on_capture_taken));
...
void MyWindow::add_debug_text(const std::string& text) {
...
bool MyWindow::ensure_captures_directory() {
    return true; // handled in Main.cpp
}

// Handle captures coming from SauronEyePanel
void MyWindow::on_capture_taken(const std::string& filename) {
...
bool MyWindow::on_key_press_event(GdkEventKey* key_event) { return false; }
bool MyWindow::on_delete_event(GdkEventAny* event) { return false; }

void MyWindow::on_mqtt_connect_clicked() {
...
void MyWindow::refresh_captures() {
...
void MyWindow::add_thumbnail(const std::string& filepath) {
    try {
        ...
        button->signal_clicked().connect(
            sigc::bind(sigc::mem_fun(*this, &MyWindow::on_thumbnail_clicked), filepath));
...
void MyWindow::on_thumbnail_clicked(const std::string& filepath) {
    if (mqtt_connected_) {
...
void MyWindow::on_open_folder_clicked() {
    std::string command;
#ifdef __linux__
...
void MyWindow::on_thumbnail_activated_capture(const std::string& filepath) {
